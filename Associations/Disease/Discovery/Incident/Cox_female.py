...
badoutfile = dpath + 'Results/Associations/Discovery/Incident/bad_tgt_female.csv'

tgt_df = pd.read_csv(dpath + 'Data/TargetData/IncidentDiseaseTable.csv')
########### remove opposite sex ##############
tgt_df = tgt_df.loc[tgt_df.SEX != 1]
...
########### retain desired sex ##############
mydf = mydf.loc[mydf.Sex == 0]
mydf.reset_index(inplace=True, drop=True)
mydf = mydf.loc[mydf.Split == 1]
mydf.reset_index(inplace = True, drop = True)

my_formula = "Age + TDI + BMI + C(Smoke) + Statin + FastingTime + x_nmr"
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Results/Associations/Discovery/Incident/Female/'+tgt_name+'.csv', index=False)
        else:
