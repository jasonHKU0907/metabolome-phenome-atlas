...
badoutfile = dpath + 'Results/Associations/Discovery/Incident/bad_tgt_male.csv'

tgt_df = pd.read_csv(dpath + 'Data/TargetData/IncidentDiseaseTable.csv')
########### remove opposite sex ##############
tgt_df = tgt_df.loc[tgt_df.SEX != 2]
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Results/Associations/Discovery/Incident/Male/' + tgt_name + '.csv', index=False)
        else:
