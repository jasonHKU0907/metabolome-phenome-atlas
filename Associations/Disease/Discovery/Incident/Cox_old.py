...
badoutfile = dpath + 'Results/Associations/Discovery/Incident/bad_tgt_old.csv'
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Results/Associations/Discovery/Incident/Old/' + tgt_name + '.csv', index=False)
        else:
