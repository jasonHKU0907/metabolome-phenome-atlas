...
badoutfile = dpath + 'Results/Associations/Discovery/Prevalent/bad_tgt_female.csv'
...
cov_f_lst = ["Age", "TDI", "BMI", "Smoke", "Statin", "FastingTime"]
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Results/Associations/Discovery/Prevalent/Female/'+tgt_name+'.csv', index=False)
        else:
