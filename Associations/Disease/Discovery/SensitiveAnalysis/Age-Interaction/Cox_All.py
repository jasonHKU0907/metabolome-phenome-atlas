...
def process(nmr_f, tmp_df, my_formula):
    ...
    tmp_df['AgeInter'] = tmp_df.Age*tmp_df.x_nmr
    ...
    while ((len(tmpout) == 0) | (i > 1e7)):
        ...
        try:
            ...
            hr = np.round(cph.hazard_ratios_.AgeInter, 5)
            lbd = np.round(np.exp(cph.confidence_intervals_).loc['AgeInter'][0], 5)
            ubd = np.round(np.exp(cph.confidence_intervals_).loc['AgeInter'][1], 5)
            pval = cph.summary.p.AgeInter
...
badoutfile = dpath + 'Revision/Results/Associations/Discovery/Incident/bad_tgt_all-ageinter.csv'
...
mydf['Age'] = (mydf.Age-mydf.Age.mean())/mydf.Age.std()

formula_full = "Age + C(Sex) + TDI + BMI + C(Smoke) + Statin + FastingTime + x_nmr + AgeInter"
formula_sex = "Age + TDI + BMI + C(Smoke) + Statin + FastingTime + x_nmr + AgeInter"
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        ...
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Revision/Results/Associations/Discovery/Incident/All-AgeInteraction/' + tgt_name + '.csv', index=False)
        else:
