...
def process(nmr_f, tmp_df, cov_f_lst):
    ...
    Y = tmp_df.target_y
    X = tmp_df[cov_f_lst + ['x_nmr', 'AgeInter']]
    try:
        try:
            log_mod = sm.Logit(Y, sm.add_constant(X)).fit()
            oratio = np.round(np.exp(log_mod.params).loc['AgeInter'], 5)
            pval = log_mod.pvalues.loc['AgeInter']
            ci_mod = log_mod.conf_int(alpha=0.05)
            lbd, ubd = np.round(np.exp(ci_mod.loc['AgeInter'][0]), 5), np.round(np.exp(ci_mod.loc['AgeInter'][1]), 5)
...
badoutfile = dpath + 'Results/Associations/Discovery/Prevalent/bad_tgt_all-ageinter.csv'
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Revision/Results/Associations/Discovery/Prevalent/All-AgeInteraction/' + tgt_name + '.csv', index=False)
        else:
