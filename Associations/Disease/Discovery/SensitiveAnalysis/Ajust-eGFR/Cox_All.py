...
badoutfile = dpath + 'Revision/Results/Associations/Discovery/Incident/bad_tgt_all.csv'
...
formula_full = "Age + C(Sex) + TDI + BMI + C(Smoke) + Statin + FastingTime + eGFR + x_nmr"
formula_sex = "Age + TDI + BMI + C(Smoke) + Statin + FastingTime + eGFR + x_nmr"
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        ...
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Revision/Results/Associations/Discovery/Incident/Adjust-eGFR/' + tgt_name + '.csv', index=False)
        else:
