...
badoutfile = dpath + 'Results/Associations/Discovery/Prevalent/bad_tgt_all.csv'
...
cov_f_lst_full = ["Age", "Sex", "TDI", "BMI", "Smoke", "Statin", "eGFR", "FastingTime"]
cov_f_lst_sex = ["Age", "TDI", "BMI", "Smoke", "Statin", "eGFR", "FastingTime"]
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Revision/Results/Associations/Discovery/Prevalent/Adjust-eGFR/' + tgt_name + '.csv', index=False)
        else:
