...
def process(nmr_f, tmp_df, cov_f_lst):
    ...
    Y = tmp_df.target_y
    X = tmp_df[cov_f_lst + ['x_nmr', 'SexInter']]
    try:
        try:
            log_mod = sm.Logit(Y, sm.add_constant(X)).fit()
            oratio = np.round(np.exp(log_mod.params).loc['SexInter'], 5)
            pval = log_mod.pvalues.loc['SexInter']
            ci_mod = log_mod.conf_int(alpha=0.05)
            lbd, ubd = np.round(np.exp(ci_mod.loc['SexInter'][0]), 5), np.round(np.exp(ci_mod.loc['SexInter'][1]), 5)
...
badoutfile = dpath + 'Results/Associations/Discovery/Prevalent/bad_tgt_all-sexiter.csv'
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(dpath + 'Revision/Results/Associations/Discovery/Prevalent/All-SexInteraction/' + tgt_name + '.csv', index=False)
        else:
