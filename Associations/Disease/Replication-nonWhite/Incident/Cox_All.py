...
def process(nmr_f, tmp_df0, my_formula):
    tmp_df = tmp_df0.copy()
...
badoutfile = dpath + 'Results/Associations/Replication_nonWhite/Incident/bad_tgt_all.csv'
...
formula_full = "Age + C(Sex) + C(Race) + TDI + BMI + C(Smoke) + Statin + FastingTime + x_nmr"
formula_sex = "Age + C(Race) + TDI + BMI + C(Smoke) + Statin + FastingTime + x_nmr"
...
for tgt_name in tqdm(tgt_lst):
    ...
    try:
        if tmp_df0.target_y.sum()>=50:
            tgt_out_df = Parallel(n_jobs=nb_cpus)(delayed(process)(nmr_f, tmp_df0, my_formula) for nmr_f in nmr_f_lst)
            #tgt_out_df = [process(nmr_f, tmp_df0, my_formula) for nmr_f in nmr_f_lst]
            ...
            tgt_out_df.to_csv(dpath + 'Results/Associations/Replication_nonWhite/Incident/All/' + tgt_name + '.csv', index=False)
        else:
