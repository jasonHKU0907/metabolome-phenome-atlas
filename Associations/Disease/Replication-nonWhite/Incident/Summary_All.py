...
group = 'All'
tgt_info_df = pd.read_csv(dpath + 'Data/TargetData/IncidentDiseaseTable.csv')
...
results_dir_lst = sort_nicely(glob.glob(dpath + 'Results/Associations/Replication_nonWhite/Incident/'+group+'/*.csv'))
...
meta_df = meta_df.loc[meta_df.Split == 3]
...
mydf.to_csv(dpath + 'Results/Associations/Replication_nonWhite/Incident/Summary/Data_Summary_'+group+'.csv', index = False)
