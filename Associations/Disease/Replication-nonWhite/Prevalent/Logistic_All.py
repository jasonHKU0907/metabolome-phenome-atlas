...
def process(nmr_f, tmp_df0, cov_f_lst):
    tmp_df = tmp_df0.copy()
...
badoutfile = dpath + 'Results/Associations/Replication_nonWhite/Prevalent/bad_tgt_all.csv'
...
mydf = mydf.loc[mydf.Split == 3]
mydf.reset_index(inplace = True, drop = True)
mydf['Race2'] = mydf['Race']
mydf['Race2'].replace([2,3,4], [1, 0, 0], inplace = True)
mydf['Race3'] = mydf['Race']
mydf['Race3'].replace([2,3,4], [0, 1, 0], inplace = True)
mydf['Race4'] = mydf['Race']
mydf['Race4'].replace([2,3,4], [0, 0, 1], inplace = True)

cov_f_lst_full = ["Age", "Race2", "Race3", "Sex", "TDI", "BMI", "Smoke", "Statin", "FastingTime"]
cov_f_lst_sex = ["Age", "Race2", "Race3", "TDI", "BMI", "Smoke", "Statin", "FastingTime"]
...
for tgt_name in tqdm(tgt_lst):
    ...
    tmp_df0 = pd.merge(mydf, tmp_tgt_df, how='inner', on=['eid'])
    try:
        if tmp_df0.target_y.sum()>=50:
            tgt_out_df = [process(nmr_f, tmp_df0, cov_f_lst) for nmr_f in nmr_f_lst]
            ...
            tgt_out_df = tgt_out_df[['NMR_code', 'nb_individuals', 'nb_case', 'prop_case(%)', 'oratio',
                                     'or_lbd', 'or_ubd', 'pval_raw', 'pval_bfi', 'or_output', 'pval_significant']]
            tgt_out_df.to_csv(dpath + 'Results/Associations/Replication_nonWhite/Prevalent/All/' + tgt_name + '.csv', index=False)
        else:
