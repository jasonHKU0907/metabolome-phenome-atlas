...
for tmp_tgt in tqdm(tgt2pred_lst):
    ...
    try:
        ...
        imp_df.to_csv(dpath + 'Results/Prediction/Prevalent/NMR_Importance/' + tmp_tgt + '.csv', index=False)
        pred_df.to_csv(dpath + 'Results/Prediction/Prevalent/Predictions/' + tmp_tgt + '.csv', index=False)
    except:
        bad_tgt_lst.append(tmp_tgt)

bad_tgt_df = pd.DataFrame({'Disease_code': bad_tgt_lst})
bad_tgt_df.to_csv(dpath + 'Results/Prediction/Prevalent/bad_tgt_df.csv', index=False)
...
cd /home1/jiayou/Documents/Projects/MetaAtlas/SeverCode/Prediction/Prevalent/
