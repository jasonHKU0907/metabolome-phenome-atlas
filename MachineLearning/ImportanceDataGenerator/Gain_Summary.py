...
for root in tqdm(root_lst):
    ...
    for name in name_lst:
        ...
        tgt_imp_df.rename(columns = {'TotalGain_cv':name}, inplace = True)
    ...
    name_imp_df.to_csv(dpath + 'Results/Prediction/'+type+'/ImportanceSummary/GainSummary/'+root_name+'.csv', index = False)
    root_imp_df = pd.merge(root_imp_df, name_imp_df[['NMR_code', root_name]], how = 'left', on = ['NMR_code'])

root_imp_df.to_csv(dpath + 'Results/Prediction/'+type+'/ImportanceSummary/GainSummary/Importance_by_root.csv', index = False)
