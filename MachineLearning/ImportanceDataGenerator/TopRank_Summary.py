...
tgt_dir_lst = sort_nicely(glob.glob(dpath + 'Results/Prediction/'+type+'/NMR_Importance/*.csv'))
...
for root in tqdm(root_lst):
    ...
    for name in name_lst:
        tgt_dir = dpath + 'Results/Prediction/'+type+'/NMR_Importance/'+name+'.csv'
        tgt_imp_df = pd.read_csv(tgt_dir, usecols = ['NMR_code', 'TotalGain_cv'])
        tgt_imp_df.sort_values(by = 'TotalGain_cv', ascending=False, inplace = True)
        tgt_imp_df[name] = [1]*top_nb + [0]*(len(tgt_imp_df)-top_nb)
        name_imp_df = pd.merge(name_imp_df, tgt_imp_df[['NMR_code', name]], how = 'left', on = ['NMR_code'])
    name_imp_df[root_name] = name_imp_df.iloc[:, 1:].sum(axis=1).tolist()
    name_imp_df.to_csv(dpath + 'Results/Prediction/'+type+'/ImportanceSummary/Top'+str(top_nb)+'_summary/'+root_name+'.csv', index = False)
...
root_imp_df.to_csv(dpath + 'Results/Prediction/'+type+'/ImportanceSummary/Top'+str(top_nb)+'_summary/Importance_by_root.csv', index = False)
