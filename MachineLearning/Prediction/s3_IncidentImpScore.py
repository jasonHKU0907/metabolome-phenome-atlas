...
tgt_dir_lst = sort_nicely(glob.glob(dpath + 'Revision/Results/Prediction/Incident/NMR_Importance/*.csv'))
...
tgt_dir = tgt_dir_lst[0]

for tgt_dir in tqdm(tgt_dir_lst):
    tgt = os.path.basename(tgt_dir)[:-4]
    tgt_name = tgt_dict.loc[tgt_dict.NAME == tgt].LONGNAME.iloc[0]
    output_img = dpath + 'Revision/Results/Prediction/Incident/Importance_Plot/' + tgt + '.pdf'
    tgt_pred_df = pd.read_csv(tgt_dir)
    tgt_pred_df = tgt_pred_df.iloc[:30,:]
    tgt_pred_df = pd.merge(tgt_pred_df, met_dict, how = 'left', left_on = 'NMR_code', right_on='Metabolite_code')
    fig, ax = plt.subplots(figsize=(12, 6))
    plt.rcParams["font.family"] = "Arial"
    palette = sns.color_palette("Blues", n_colors=len(tgt_pred_df))
    palette.reverse()
    sns.barplot(ax=ax, x="plot_code", y="Importance", palette=palette, data=tgt_pred_df.sort_values(by="Importance", ascending=False))
    y_imp_up_lim = round(tgt_pred_df['Importance'].max() + 0.005, 2)
    ...
    ax.set_ylabel('Normalized metabolite importance', weight='bold', fontsize=14)
