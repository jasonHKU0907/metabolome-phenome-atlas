...
tgt_dir_lst = sort_nicely(glob.glob(dpath + 'Revision/Results/Prediction/Prevalent/Predictions/*.csv'))
...
for tgt_dir in tqdm(tgt_dir_lst):
    ...
    output_img = dpath + 'Revision/Results/Prediction/Prevalent/AUC_plot/' + tgt + '.pdf'
    eval_df = pd.read_csv(dpath + 'Revision/Results/Prediction/Prevalent/Evaluation/' + tgt + '.csv')
    ...
    for fold_id in region_fold_id_lst:
        try:
            ...
            y_pred1, y_pred2, y_pred3 = tgt_pred_df.iloc[test_idx].y_pred_cov, tgt_pred_df.iloc[test_idx].y_pred_nmr, tgt_pred_df.iloc[test_idx].y_pred_nmr_cov
